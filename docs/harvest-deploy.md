...
代码通过 Git 保存；生产服务器沿用 rsync 上传到 `root@47.114.78.196:/var/www/agri-monitor/`，然后 PM2 重启，不使用服务器 Git 拉取，也不使用 `--delete`。
...
```sh
ssh root@47.114.78.196 'mkdir -p /var/backups/agri-monitor && tar -czf /var/backups/agri-monitor/before-harvest-$(date +%Y%m%d-%H%M%S).tgz -C /var/www/agri-monitor app.js index.html server.js'
```
...
rsync -avz \
  ...
  root@47.114.78.196:/var/www/agri-monitor/
```
...
rsync -avz \
  ...
  root@47.114.78.196:/var/www/agri-monitor/server-data/china-soil/
```
...
```sh
ssh root@47.114.78.196 'bash -se' <<'REMOTE'
...
完成后打开 http://47.114.78.196/?page=harvest ，登录原账号，选一个国内地点，确认能读取土壤并完成试算。页面登录不需要创建新账号。健康接口只能确认服务存活，登录后的页面操作用于确认完整功能。
